...
function initScrollSpy() {
  ...
  const navLinks = document.querySelectorAll('.nav a'); // Only nav links, not brand elements
  ...
  function updateActiveNav() {
    ...
    // If no section is detected, check if we're at the top
    if (!current && window.scrollY < 200) {
      current = 'hero';
    }
    
    // Update only navigation links (not brand elements)
    navLinks.forEach(link => {
      link.classList.remove('active');
      const href = link.getAttribute('href');
      
      if (href === `#${current}` || (href === '#hero' && current === 'hero')) {
